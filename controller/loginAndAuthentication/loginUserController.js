...
const userLogin = async (req, res) => {
  ...
  try {
    ...
    if (username || enrolmentId) {
      let recordExist = null;
      if (username) {
        console.log(`in usernamae block `);
        console.log(`username from request is ${username}`);
        console.log(`password from request is ${password}`);
        recordExist = await getUserDetailByUsername(username);
      }
      if (enrolmentId) {
        console.log(`enrolmentId from request is ${enrolmentId}`);
        recordExist = await getUserDetailByEnrolmentId(enrolmentId);
      }
